...
console.log("[Updater] Функция updateDeviceRegisters запущена");
  ...
  try {
    ...
    console.log(`[Updater] Найдено строк <tr> для опроса: ${rows.length}`);

    console.log(`[Updater] Найдено строк по селектору #grid-data-rows tr: ${rows.length}`);
    ...
    console.log(`[Updater] Уникальных адресов регистров собрано: ${uniqueAddresses.length}`);
    ...
    for (const batch of batches) {
      ...
      try {
        ...
        const reply = await serialManager.executeTransaction(
          finalPacket,
          checkComplete,
          300,
        );
                  ...
                  try {
                    ...
                    if (dataType === "TByte" || dataType === "TPrmList") {
                      if (sub === "H") {
                        ...
                        hexValue =
                          "x" +
                          byteVal.toString(16).padStart(originalHexLen, "0");
                      } else {
                    ...
                    console.error(
                      `Ошибка обработки строки для регистра 0x${regAddr.toString(16)}:`,
                      e,
                    );
                  }
                }
