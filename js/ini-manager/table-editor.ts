...
// Расширяем интерфейс HTMLElement, чтобы TS знал про blurEditor
...
export function initHexCellEditor(
    ...
    cellHex.addEventListener('click', (e: Event) => {
        ...
        let currentRawHex = parts[hexIndex] || (cellHex.textContent?.startsWith('0x') ? 'x' + cellHex.textContent.slice(2) : 'x0000') || 'x0000';
...
export function initPhysicalCellEditor(
    ...
    prmListOptions: Record<string, string>, 
    updateRowValues: Function, 
    hexToFloat32: Function, 
    float32ToHex: (val: number, len: number) => string // <-- ВОТ ТУТ ИЗМЕНЕНИЕ
): void {
    cellPhysical.addEventListener('click', (e: Event) => {
        if (cellPhysical.classList.contains('is-editing-cell')) {
            if (dataType === 'TPrmList') {
                const selectEl = cellPhysical.querySelector('.table-prm-select') as HTMLSelectElement;
                if (selectEl && selectEl.style.display === 'none') {
                    const displayEl = cellPhysical.querySelector('.prm-val-display') as HTMLElement;
        ...
        if (dataType === 'TPrmList') {
            ...
            let currentText = (displayEl?.textContent || cellPhysical.textContent || '0').trim();
