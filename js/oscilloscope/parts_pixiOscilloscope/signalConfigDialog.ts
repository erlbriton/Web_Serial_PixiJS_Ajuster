...
// Расширяем интерфейс MonitorRow только свойством autoScale,
// так как maxScale уже объявлен в базовом MonitorRow как number
...
export function openSignalConfigDialog(
    ...
    // По умолчанию считаем, что авто-масштаб включен, если иное не задано ранее
    ...
    dialog.innerHTML = `
        ...
        <div style="margin-bottom:10px;">
            ...
            <textarea id="osc-cfg-desc" rows="2" ${disabledAttr} style="${inputStyle} resize:none;">${signal.description || ''}</textarea>
        </div>
        ...
        ${isTBit 
            ? `<button id="osc-close-btn" style="width:100%; padding:8px; background:#444; color:white; border:none; cursor:pointer; font-weight:bold; border-radius:3px;">Закрыть</button>`
            : `<button id="osc-apply-config" style="width:100%; padding:8px; background:#0055ff; color:white; border:none; cursor:pointer; font-weight:bold; border-radius:3px;">Применить</button>`
        }
    `;
    ...
    if (isTBit) {
        ...
        // Логика блокировки поля "Максимум" при включенном "Авто"
        autoCheckbox.addEventListener('change', () => {
            maxInput.disabled = autoCheckbox.checked;
        });

        const applyBtn = document.getElementById('osc-apply-config')!;
